...
export function TileLayout({
  ...
  const agentVideoTrack = useMemo(() => {
    ...
    return cameraTracks.find(
      (t) =>
        t.participant.identity.includes('bey-avatar') || t.participant.identity.includes('avatar')
    );
  }, [voiceAssistantVideoTrack, cameraTracks]);
        ...
        <div className={cn(tileViewClassNames.grid)}>
          ...
          >
            <AnimatePresence mode="popLayout">
              {!isAvatar && (
                // Audio Agent (with static avatar fallback)
                <motion.div
                  key="agent"
                  layoutId="agent"
                  initial={{ opacity: 0 }}
                  ...
                  className={cn(
                    'relative flex aspect-square items-center justify-center overflow-hidden rounded-full drop-shadow-xl/80',
                    avatarUrl
                ...
                >
                  ...
                  {/* Subtle dark overlay only in chat mode so avatar stays visible */}
                  {avatarUrl && chatOpen && (
                    <div className="pointer-events-none absolute inset-0 rounded-full bg-black/40" />
                  )}
                  <AudioVisualizer
                    key="audio-visualizer"
                    initial={{ scale: 1 }}
                    animate={{ scale: chatOpen ? 0.2 : avatarUrl ? 0.65 : 1 }}
                    transition={{
                      ...ANIMATION_TRANSITION,
                      delay: animationDelay,
                    }}
                    ...
                    className={cn(
                      ...
                      avatarUrl ? 'bg-transparent' : 'bg-background',
