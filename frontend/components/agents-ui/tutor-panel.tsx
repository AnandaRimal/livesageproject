...
export function TutorPanel() {
  ...
  const [viewMode, setViewMode] = useState<'pdf-page' | 'whiteboard' | 'diagrams' | 'split'>('pdf-page');
  ...
  const getStatusBadge = () => {
    switch (status) {
      case 'starting':
        return { label: 'Initializing', color: 'bg-amber-500/20 text-amber-300 border-amber-500/30' };
      case 'teaching':
        return { label: 'Teaching', color: 'bg-violet-500/20 text-violet-300 border-violet-500/30' };
      case 'advancing':
        return { label: 'Next Section', color: 'bg-cyan-500/20 text-cyan-300 border-cyan-500/30' };
      case 'complete':
        return { label: 'Class Complete', color: 'bg-emerald-500/20 text-emerald-300 border-emerald-500/30' };
      default:
        return { label: 'Active Session', color: 'bg-blue-500/20 text-blue-300 border-blue-500/30' };
    }
  };
  ...
  return (
    <div className="flex h-full w-full flex-col gap-3 rounded-2xl border border-white/10 bg-black/50 p-4 backdrop-blur-xl font-sans overflow-hidden">
      {/* Header */}
      <div className="flex items-center justify-between border-b border-white/10 pb-3">
        <div className="flex items-center gap-2">
          <div className="flex size-8 items-center justify-center rounded-xl bg-violet-600/30 text-violet-400 border border-violet-500/30">
            <GraduationCap className="size-4" />
          </div>
          <div>
            <h3 className="text-xs font-bold tracking-wider text-slate-100 uppercase flex items-center gap-1.5">
        ...
        <div className="flex items-center gap-2">
          {/* View Mode Switcher */}
          <div className="flex items-center rounded-lg bg-white/5 border border-white/10 p-0.5 text-[11px]">
            <button
              onClick={() => setViewMode('pdf-page')}
              className={cn(
                'flex items-center gap-1 px-2.5 py-1 rounded-md transition font-medium cursor-pointer',
                viewMode === 'pdf-page'
                  ? 'bg-violet-600/40 text-violet-200 shadow-sm border border-violet-500/40'
              ...
              <>
                <button
                  ...
                  className={cn(
                    'flex items-center gap-1 px-2.5 py-1 rounded-md transition font-medium cursor-pointer',
                    viewMode === 'diagrams'
                      ? 'bg-cyan-600/40 text-cyan-200 shadow-sm border border-cyan-500/40'
                      : 'text-slate-400 hover:text-slate-200'
                  )}
                  title="Diagram View"
                >
      ...
      <div className="rounded-xl border border-white/10 bg-white/5 p-3 space-y-2">
        <div className="flex items-center justify-between text-[11px] font-semibold text-slate-400">
          ...
          {pageNum && (
            <span className="rounded-md bg-violet-500/10 px-2 py-0.5 text-[10px] font-mono text-violet-300 border border-violet-500/20">
              PDF Page {pageNum}
            </span>
          )}
        </div>
      ...
      {/* Main Content Body */}
      <div className="flex flex-1 overflow-hidden rounded-xl border border-white/10 bg-black/70 relative">
        <AnimatePresence mode="wait">
          {viewMode === 'pdf-page' && (
            ...
            >
              <div className="flex items-center justify-between border-b border-white/10 bg-white/5 px-3 py-2">
                ...
                {pageImage && (
                  <button
                    ...
                    className="flex items-center gap-1 rounded-md bg-white/10 px-2 py-1 text-[10px] font-semibold text-slate-300 hover:bg-white/20 transition cursor-pointer"
                  >
              ...
              <div className="flex-1 relative flex items-center justify-center p-3 overflow-hidden bg-black/80">
                {pageImage ? (
                  <img
                  ...
                  <div className="flex flex-col items-center justify-center text-center p-6 text-slate-500 space-y-2">
                    ...
                    <p className="text-xs font-medium text-slate-400">Loading original PDF page...</p>
                  </div>
                )}
              </div>
            </motion.div>
          ...
          {viewMode === 'whiteboard' && (
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-4 font-mono text-xs leading-relaxed text-slate-200 space-y-3 bg-gradient-to-b from-slate-950/80 to-black/90">
                <div className="rounded-lg border border-violet-500/20 bg-violet-950/20 p-3 text-violet-200">
                  <p className="text-[11px] font-sans font-semibold text-violet-300 flex items-center gap-1">
                    <GraduationCap className="size-3.5" /> Professor Sage is teaching this section in Nepali:
                  </p>
                </div>
                <div className="whitespace-pre-wrap rounded-xl border border-white/10 bg-white/5 p-4 text-slate-300 font-sans text-xs leading-relaxed shadow-inner">
                  {whiteboardText}
                </div>
              </div>
            </motion.div>
          ...
          {viewMode === 'diagrams' && (
            ...
            >
              ...
              {currentImages.length > 1 && (
                <div className="flex gap-2 border-t border-white/10 bg-white/5 p-2 overflow-x-auto">
                  {currentImages.map((img, idx) => (
                    <button
                      key={img}
                      onClick={() => setSelectedImage(img)}
                      className={cn(
                        'relative h-12 w-16 shrink-0 overflow-hidden rounded-md border transition cursor-pointer',
                        selectedImage === img
          ...
          {viewMode === 'split' && (
            <motion.div
              ...
              className="flex h-full w-full flex-col md:flex-row overflow-hidden divide-y md:divide-y-0 md:divide-x divide-white/10"
            >
              ...
              <div className="flex flex-1 flex-col overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto p-3 text-xs leading-relaxed text-slate-300 bg-slate-950/60 font-sans space-y-2">
                  <p className="whitespace-pre-wrap rounded-lg border border-white/10 bg-white/5 p-3 text-slate-300 text-[11px] leading-relaxed">
                    {whiteboardText}
                  </p>
                </div>
              </div>
        ...
        {expandedModalImage && (
          ...
          >
            ...
            >
              <button
                onClick={() => setExpandedModalImage(null)}
                className="absolute top-4 right-4 z-10 flex size-8 items-center justify-center rounded-full bg-black/60 text-white hover:bg-black transition cursor-pointer"
              >
                <X className="size-4" />
              </button>
              <img
